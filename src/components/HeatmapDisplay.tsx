...
export function HeatmapDisplay({ words }: HeatmapDisplayProps) {
  // Filter to only English words (no Chinese characters)
  const englishWords = words.filter(word => /^[a-zA-Z\s\-']+$/.test(word.word));
  const sorted = [...englishWords].sort((a, b) => b.score - a.score).slice(0, 25);
